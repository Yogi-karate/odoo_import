...
class SaleOrder(Module):
	...
	def create(self,sale_list,odoo):
		...
		duplicate_list = odoo.execute_kw(self._name,'search_read',[[('name','=',[order['name'] for order in sale_list])]],{'fields':['id','name']})
		result = {x['name']:x['id'] for x in duplicate_list} 
		print("sale order cache",len(result),result)
		so_list=[]
		for so in sale_list:
			if result.get(so.get('name')):
				continue
			else:
				print("not in cache",so)
				so['order_line'] = self.create_order_line(so)
				so_list.append(so)
		_logger.info("Creating New %s sale orders in Odoo",len(so_list))
